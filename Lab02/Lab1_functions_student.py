...
def get_filter_banks(num_filters, num_FFT, sample_rate, freq_min=0, freq_max=None):
    ...
    for i in range(1, num_filters + 1):
        f_i_left = int(bins[i - 1])
        f_i = int(bins[i])
        f_i_right = int(bins[i + 1])

        for k in range(f_i_left, f_i):
            fbanks[i - 1, k] = (k - bins[i - 1]) / (bins[i] - bins[i - 1])
        for k in range(f_i, f_i_right):
            fbanks[i - 1, k] = (bins[i + 1] - k) / (bins[i + 1] - bins[i])
    ###################
